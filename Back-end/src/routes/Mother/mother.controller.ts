 ...
 * @param res Response, retorna un un object con sucess:true, data:{ _id: ObjectId() } y un message: "String" de la nueva madre si todo sale bien
 */
export const newMother: RequestHandler = async (req, res) => {
    ...
    if ( !name || !rut || !commune || !phone_number || !birth || !ocupation || !studies || 
        ...
        return res.status(400).send({ sucess: false, data:{}, message:'ERROR: Datos inválidos' + req.body });

    const motherFound = await Mother.findOne({ rut });

    //se valida la existencia de la madre en el sistema
    if ( motherFound )
        return res.status(301).send({ sucess: false, data:{}, message:'ERROR: La madre ya está registrada en el sistema.' });
    ...
    return res.status(201).send({ sucess: true, data: { _id: motherSaved._id }, message: 'Madre agregada con éxito al sistema.' });
}
 ...
 * @param res Response, retorna un un object con sucess:true, data:{} y un message: "String" de la madre editada si todo sale bien
 */
export const editMother: RequestHandler = async (req, res) => {
    ...
    if ( !Types.ObjectId.isValid(_id) )
        return res.status(400).send({ sucess: false, data:{}, message: 'ERROR: El id ingresado no es válido.' });
    
    const motherFound = await Mother.findById( _id );

    //se valida la existencia de la madre en el sistema
    if ( !motherFound ) 
        return res.status(404).send({ sucess: false, data:{}, message: 'ERROR: La madre ingresada no existe en el sistema.' });

    //se actualiza la madre en el sistema
    await Mother.findByIdAndUpdate( _id, updatedMother );

    return res.status(200).send({ sucess: true, data:{}, message: 'Madre editada de manera correcta.' });
}
 ...
 * @param res Response, retorna un un object con sucess:true, data:{  mother:{} } y un message: "String" de la madre obtenida si todo sale bien
 */
export const getDetailedMother: RequestHandler = async (req, res) => {
    ...
    if( !motherFound )
        return res.status(404).send({ sucess: false, data:{}, message:'ERROR: La madre a obtener no existe en el sistema.' });

    //se seleccionan los atributos que se van a mandar al front
        const motherFiltered = destructureMother( motherFound );

    return res.status(200).send({ sucess: true, data:{ mother: motherFiltered }, message:'Madre encontrada y obtenida de manera correcta.' })
}
 ...
 * @param res Response, retorna un un object con sucess:true, data:{  listMothers:[{},{},...] } y un message: "String" de las madres obtenidas si todo sale bien
 */
export const getMothers: RequestHandler = async (req, res) => {
    ...
    if ( !mothersFound )
        return res.status(404).send({ sucess: false, data:{}, message:'ERROR: No existen madres registradas en el sistema.' });
    ...
    return res.status(200).send({ sucess: true, data:{ list_of_mothers: listMothers }, message: "Se obtuvieron a todas las madres del sistema de manera exitosoa." });
}
